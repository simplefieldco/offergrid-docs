...
```bash
curl -X PATCH "https://offergrid-api.onrender.com/provider/orders/ITEM_ID/status" \
...
async function onInstallationScheduled(workOrder) {
  await fetch(`https://offergrid-api.onrender.com/provider/orders/${workOrder.offergridItemId}/status`, {
    method: 'PATCH',
    headers: {
